...
def ensure_column():
    ...
    if "ir_url" not in cols:
        print("[info] companies.ir_url 컬럼 추가")
        db.execute("ALTER TABLE companies ADD COLUMN ir_url TEXT")
    if "ir_url_fetched_at" not in cols:
        db.execute("ALTER TABLE companies ADD COLUMN ir_url_fetched_at TEXT")
    db.commit(); db.close()
    ...
    try:
        ...
        if d.get("status") == "000":
            return {"ok": True, "corp_code": corp_code,
                    "ir_url": (d.get("ir_url") or "").strip(),
                    "hm_url": (d.get("hm_url") or "").strip()}
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for i, fut in enumerate(as_completed(futs), 1):
            ...
            if r["ok"]:
                ir_url = r["ir_url"]
                if ir_url: with_eng += 1
                db.execute(
                    "UPDATE companies SET ir_url=?, ir_url_fetched_at=? WHERE corp_code=?",
                    [ir_url, now, cc]
                )
                ok_cnt += 1
            else:
            ...
            if i % 100 == 0:
                ...
                print(f"  [{i:>4}/{total}] ok={ok_cnt} err={err_cnt} ir_url={with_eng} (남은 ~{eta:.0f}s)")
    db.commit()
    db.close()
    print(f"\n[완료] {ok_cnt}/{total} 성공, ir_url 있음 {with_eng}, 오류 {err_cnt}")
